...
- **`<artefact-type>`** — the artefact category (e.g. `Personas`, `Journeys`, `Research-synthesis`, `Problem-impact-analysis`, `Prior-knowledge`, `PRDs`, `Before-after-journeys`, `Interview-guides`, `Capability-storyboards`, `Test-plans`, `Field-notes`), a page whose `parent_id` is the track node. The artefact brief tells you which value to use.
